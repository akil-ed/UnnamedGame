...
public class GridMove_m : MonoBehaviour {
	...
	public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		if (stream.isWriting) 
		{
			stream.Serialize (ref m_startPosition);
			stream.Serialize (ref m_endPostion);
			stream.Serialize (ref m_moveSpeed);
			stream.Serialize (ref m_movementTime);
//			stream.Serialize (ref m_);
		}
		else
		{
			stream.Serialize (ref m_startPosition);
