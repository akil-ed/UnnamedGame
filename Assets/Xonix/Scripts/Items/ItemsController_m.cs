...
public class ItemsController_m : MonoBehaviour {
	...
	[SerializeField]
	private List<string> m_specialItems;
	...
	private void GenerateSpecialItems () 
	{
		...
		Transform i = PhotonNetwork.Instantiate(m_specialItems[index], centroid, Quaternion.identity,new byte()).transform;

		float i_height = i.GetComponent<Collider>().bounds.center.y - i.GetComponent<Collider>().bounds.min.y;

		ItemBehaviour_m iB = i.GetComponent<ItemBehaviour_m> ();
